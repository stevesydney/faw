...
if ( ! function_exists( 'faw_posted_on' ) ) :
	...
	function faw_posted_on() {
		...
		if ( get_the_time( 'U' ) !== get_the_modified_time( 'U' ) ) {
			$time_string = '<time class="article-published" datetime="%1$s" itemprop="dateCreated pubdate datePublished">%2$s</time><time class="article-modified" datetime="%3$s" itemprop="dateModified">%4$s</time>';
		}
	...
	function faw_entry_footer() {
		...
		if ( 'post' === get_post_type() ) {
			...
			if ( $categories_list ) {
				...
				printf( '<span class="cat-links">' . esc_html__( 'Posted in %1$s', 'faw' ) . '</span>', $categories_list ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
			}

			/* translators: used between list items, there is a space after the comma */
			$tags_list = get_the_tag_list( '', esc_html_x( ', ', 'list item separator', 'faw' ) );
			if ( $tags_list ) {
			...
			comments_popup_link(
				sprintf(
					wp_kses(
						...
						__( 'Leave a Comment<span class="screen-reader-text"> on %s</span>', 'faw' ),
						array(
							'span' => array(
								'class' => array(),
							),
